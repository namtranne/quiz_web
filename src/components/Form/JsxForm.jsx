			...
			<fieldset>
				...
				<div className="grid grid-cols-4 sm:grid-cols-2 gap-2">
					{categoriesJSON.map(category => (
						<label key={category.id} className="relative cursor-pointer aspect-square" title={category.name}>
							<input
							...
							<Image
								className={`absolute inset-0 m-2 object-contain transition-all rounded
									peer-checked:scale-90
									peer-checked:bg-[${category.color}]
									invert peer-checked:invert-0
									peer-checked:outline-2 peer-checked:outline-offset-2
									peer-checked:outline outline-[var(--bgColor)]
									`}
								src={`/categories-icons/${category.name.toLowerCase()}.png`}
								alt={category.name}
								width={40}
								height={40}
